...
const DRIPPING = (destination) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const workbook = node_xlsx_1.default.parse(destination.replaceAll("\\", "/"), { raw: true });
        const worksheet = workbook.filter((item) => item.name == "DRIPPING")[0];
        ...
        yield Promise.all(data_dict.map((item, index) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            if (validate.error) {
                ...
                yield client_1.db.sTI_Quality_FQC_TestReport_Dripping.create({
                    data: Object.assign({}, data_to_store),
                });
                finished.push(index);
            }
            myLog(`Process row DRIPPING: ${finished.length} / ${data_count} (${((finished.length / data_count) * 100).toFixed(2)} %)`);
        })));
    }
    catch (error) {
        console.log(error);
